...
class TestParamNameEndsWithType:
    """Tests for _check_param_name_ends_with_type (Check 111)."""
    ...
    def test_string_suffix_fires(self):
        issues = _check_param_name_ends_with_type("tool", self._schema("query_string"))
        assert len(issues) == 1
        assert issues[0].check == "param_name_ends_with_type"
        assert issues[0].severity == "warn"

    def test_boolean_suffix_fires(self):
        issues = _check_param_name_ends_with_type("tool", self._schema("active_boolean"))
        assert len(issues) == 1
        assert "active_boolean" in issues[0].message

    def test_integer_suffix_fires(self):
        issues = _check_param_name_ends_with_type("tool", self._schema("limit_integer"))
        assert len(issues) == 1

    def test_array_suffix_fires(self):
        issues = _check_param_name_ends_with_type("tool", self._schema("tags_array"))
        assert len(issues) == 1

    def test_object_suffix_fires(self):
        issues = _check_param_name_ends_with_type("tool", self._schema("config_object"))
        assert len(issues) == 1

    def test_descriptive_name_passes(self):
        issues = _check_param_name_ends_with_type("tool", self._schema("query", "limit", "active"))
        assert issues == []
...
class TestNameUsesCamelcase:
    """Tests for _check_name_uses_camelcase (Check 113)."""
    ...
    def test_camelcase_tool_name_fires(self):
        issues = _check_name_uses_camelcase("getUserById", {}, self._schema("id"), "mcp")
        tool_issues = [i for i in issues if "tool name" in i.message]
        assert len(tool_issues) == 1
        assert tool_issues[0].check == "name_uses_camelcase"
        assert tool_issues[0].severity == "warn"

    def test_camelcase_param_fires(self):
        issues = _check_name_uses_camelcase("get_user", {}, self._schema("userId"), "mcp")
        param_issues = [i for i in issues if "param" in i.message]
        assert len(param_issues) == 1
        assert "userId" in param_issues[0].message

    def test_snake_tool_name_passes(self):
        issues = _check_name_uses_camelcase("get_user_by_id", {}, self._schema("user_id"), "mcp")
        assert issues == []

    def test_snake_param_name_passes(self):
        issues = _check_name_uses_camelcase("create_item", {}, self._schema("created_at", "max_retries"), "mcp")
        assert issues == []

    def test_suggests_snake_case(self):
        issues = _check_name_uses_camelcase("getUser", {}, {}, "mcp")
        assert "get_user" in issues[0].message
